...
class ExportDialog extends JDialog implements ActionListener {
	...
	private void export() {
		...
		formatChoice = formatMenu.getSelectedItem().substring(formatLength - 5, formatLength - 1);

		// Make sure file has correct extension
		String filePath = file.getAbsolutePath();
		if (!filePath.endsWith(formatChoice)) {
			filePath += formatChoice;
		}

		fileChoice = new File(filePath);
	...
	private JPanel getFilePanel() {
		...
		browseButton.setPreferredSize(
				new Dimension(browseButton.getPreferredSize().width, fileField.getPreferredSize().height));
